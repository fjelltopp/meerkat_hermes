...
def require_api_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not key and app.config["API_KEY"]:
            app.logger.warning('No api_key specified, checking auth instead.')
            auth.check_auth(['hermes'], ['meerkat'])
            return f(*args, **kwargs)

        elif key == app.config["API_KEY"] or not app.config["API_KEY"]:
            app.logger.warning('Using specified API Key to authenticate.')
            return f(*args, **kwargs)

        else:
            logging.warning('Incorrect API Key provided.')
            app.logger.warning(
